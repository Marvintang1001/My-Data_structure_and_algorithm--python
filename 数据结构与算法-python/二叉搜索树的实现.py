...
class TreeNode(object):
    ...
    def spliceOut(self):
        ...
        if self.isLeaf():  # 如果它是一个叶子
            if self.isLeftChild():
                self.parent.leftChild = None
            else:
                self.parent.rightChild = None
        elif self.hasAnyChildren():  # 如果它有左孩子或右孩子
            if self.hasLeftChild():
                if self.isLeftChild():
    ...
    def remove(self, currentNode):  # delete函数的辅助函数
        if currentNode.isLeaf():  # this node is leaf
            if currentNode == currentNode.parent.leftChild:
                currentNode.parent.leftChild = None  # 把它爸的孩子记录删掉
            else:
...
if __name__ == '__main__':
    ...
    tree.levelOrder()
    del tree[28]
    print('finish delete 28')
    tree.levelOrder()
    print('-----------')
    print('31的floor = %s' % tree.getFloorAndCeil(31)[0])
    print('31的ceil = %s' % tree.getFloorAndCeil(31)[1])
